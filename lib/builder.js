...
class Builder {
	...
	generate_report() {
		...
		console.log(`\t\t${sorted_regions.join("\t")}`);
		sorted_regions.forEach((region) => {
			...
			const counts = logs.reduce((aggregate, entry) => {
				...
				aggregate[region].total_time = aggregate[region].total_time || 0;
				aggregate[region].total_time += entry.total_time;
				return aggregate;
			}, {});
			const averages = sorted_regions.reduce((aggregate, region) => {
				aggregate[region] = counts[region].total_time / counts[region].count;
				return aggregate;
			}, {});
			const printable = sorted_regions.map((region) => { return Math.round(averages[region]); }).join("\t\t");
			console.log(`${region}\t${printable}`);
		});
	}
