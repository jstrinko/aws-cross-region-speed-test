...
class Builder {
	...
	build_hosts() {
		...
		return fs.promises.writeFile('/tmp/hosts.json', JSON.stringify(this.hosts));
	}
		...
		return scp({
			...
			.then((client) => {
				return client.uploadDir(`${__dirname}/../ec2-src`, '/home/ec2-user')
					.then((response) => {
						console.warn("SCP RES:", response);
						client.uploadFile('/tmp/hosts.json', '/home/ec2-user/ec2-src/')
							.then((response) => {
								console.warn("FILE RES:", response);
								client.close();
							});
					});
			});
